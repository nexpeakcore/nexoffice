  ...
  const document =
    kind === 'xlsx'
      ? `an Excel workbook named "${name}". Use the tools to inspect it before answering questions about its contents; never guess cell values. ` +
        ...
        'Their conditions are operators or functions applied across ranges — =FILTER(A2:C100,(B2:B100="Hanoi")*(C2:C100>5)), =FILTER(A2:C100,ISNUMBER(SEARCH("x",B2:B100))) — a function given a range where it takes one value runs once per cell (=LEN(B2:B100), =IF(C2:C100>5,"high","low"), =VLOOKUP(A2:A100,Table,2,FALSE)), and their results feed any function that takes a range: =SUM(FILTER(…)), =ROWS(FILTER(…)), =COUNTA(UNIQUE(…)), =TEXTJOIN(", ",TRUE,UNIQUE(…)). ' +
        'One rule about them: the cells a result needs must be empty or the whole result is #SPILL!, so anchor one where there is room. ' +
