  ...
  const document =
    kind === 'xlsx'
      ? `an Excel workbook named "${name}". Use the tools to inspect it before answering questions about its contents; never guess cell values. ` +
        ...
        'Their conditions are operators applied across ranges — =FILTER(A2:C100,(B2:B100="Hanoi")*(C2:C100>5)) — and their results feed any function that takes a range: =SUM(FILTER(…)), =ROWS(FILTER(…)), =COUNTA(UNIQUE(…)), =TEXTJOIN(", ",TRUE,UNIQUE(…)). Scalar functions do not yet run across a range, so =LEN(B2:B100) or =ISNUMBER(SEARCH("x",B2:B100)) is #VALUE!; write the condition with operators instead. ' +
